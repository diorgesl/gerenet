...
| 2 | O separador entre blocos é a linha `#`, ignorada antes da regra de contexto; `sysname`, `return` e blocos de topo que não são `interface`/`bgp` não interferem. | As linhas `#` estão lá, e nenhum bloco de topo vem indentado. |
